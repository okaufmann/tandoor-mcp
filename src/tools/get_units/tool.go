...
func Register(server *mcp_sdk.Server, client *tandoor.Client) {
	mcp_sdk.AddTool(server, &mcp_sdk.Tool{
		Name:        "get_units",
		Description: "List or search for units of measurement.",
	}, func(ctx context.Context, req *mcp_sdk.CallToolRequest, args Args) (*mcp_sdk.CallToolResult, any, error) {
		log.Printf("Executing get_units")

		res, err := unit.ListUnits(ctx, client, unit.ListUnitsParams{
			Query:    args.Query,
			Page:     args.Page,
		...
		if err != nil {
			return &mcp_sdk.CallToolResult{
				Content: []mcp_sdk.Content{
					&mcp_sdk.TextContent{Text: fmt.Sprintf("Error listing units: %v", err)},
				},
				IsError: true,
			}, nil, nil
		}
